...
export default function StatesEngine() {
  ...
  const getStateInfo = () => {
    ...
    const score = automation.ultra_score;
    ...
    if (score >= 80) {
      ...
      return {
        ...
        priority: automation.weakest_hub?.name || 'Weakest Hub',
        dangerLevel: 'Low',
        icon: Activity,
      ...
      return {
        ...
        priority: automation.weakest_hub?.name || 'Emergency Hub',
        dangerLevel: 'High',
        icon: AlertTriangle,
      ...
      return {
        ...
        priority: automation.weakest_hub?.name || 'Critical Hub',
    ...
    <div className="space-y-6">
            ...
            <div className="space-y-4">
              <div className="text-center">
                ...
                <div className="flex items-center justify-center gap-2 text-sm">
                  {automation.score_trend > 0 ? (
                    <>
                      <TrendingUp className="w-4 h-4 text-green-600" />
                      <span className="text-green-600">+{automation.score_trend.toFixed(1)} (7-day trend)</span>
                    </>
                  ) : automation.score_trend < 0 ? (
                    <>
                      <TrendingDown className="w-4 h-4 text-red-600" />
                      <span className="text-red-600">{automation.score_trend.toFixed(1)} (7-day trend)</span>
                    </>
                  ) : (
      ...
      <div className="grid md:grid-cols-2 gap-4">
            ...
            {isLoading ? (
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className={`text-xl font-bold ${automation.weakest_score < 30 ? 'text-red-600' : 'text-orange-600'}`}>
                    {automation.weakest_score.toFixed(1)}
                  </span>
                </div>
                {automation.weakest_score < 20 && (
                  <Badge variant="destructive" className="gap-1">
                    <AlertOctagon className="w-3 h-3" />
                    CRITICAL HUB
                  </Badge>
                )}
                {automation.weakest_score >= 20 && automation.weakest_score < 30 && (
                  <Badge variant="destructive" className="gap-1">
        ...
        <Card className="border-2 border-primary/30">
          ...
          <CardContent>
            {isLoading ? (
              <Skeleton className="h-20 w-full" />
            ) : automation?.priority_hub ? (
              <div className="space-y-3">
                <div className="text-2xl font-bold">{automation.priority_hub.name}</div>
                <div className="space-y-2">
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-muted-foreground">Priority Score</span>
                    <span className="font-semibold">{automation.priority_score.toFixed(1)}</span>
                  </div>
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-muted-foreground">Zone</span>
                    <span className="font-semibold">{automation.priority_zone}</span>
                  </div>
                </div>
              </div>
            ) : (
      ...
      <Card className="border-2 border-accent/30">
        ...
        <CardContent>
          ...
          ) : automation?.focus_recommendations?.suggested_actions.length ? (
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="space-y-1">
                ...
                <p className={`text-2xl font-bold ${automation.hubs_in_danger > 0 ? 'text-red-600' : 'text-green-600'}`}>
                  {automation.hubs_in_danger}
                </p>
              </div>
              <div className="space-y-1">
                <p className="text-xs text-muted-foreground">Hub Imbalance</p>
                <p className="text-2xl font-bold">{automation.hub_imbalance.toFixed(0)}</p>
              </div>
              <div className="space-y-1">
                <p className="text-xs text-muted-foreground">Habit Consistency</p>
                <p className="text-2xl font-bold">{automation.habit_consistency.toFixed(1)}</p>
              </div>
              <div className="space-y-1">
                <p className="text-xs text-muted-foreground">Calendar Load</p>
                <p className="text-2xl font-bold">{automation.calendar_load}</p>
              </div>
              <div className="space-y-1">
                <p className="text-xs text-muted-foreground">Recent Activity</p>
                <p className="text-2xl font-bold">{automation.recent_activity}</p>
              </div>
              <div className="space-y-1">
                <p className="text-xs text-muted-foreground">Score Trend</p>
                <p className={`text-2xl font-bold ${automation.score_trend > 0 ? 'text-green-600' : automation.score_trend < 0 ? 'text-red-600' : ''}`}>
