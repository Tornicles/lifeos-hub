...
export default function Calendar() {
  ...
  const { data: entries, isLoading } = useQuery({
    queryKey: ['calendar-entries', currentDate.toISOString().split('T')[0]],
    queryFn: async () => {
  ...
  const getFocusDomainColor = (domain: string | null) => {
                ...
                <CardContent className="space-y-2">
                  {dayEntries.length > 0 ? (
                    dayEntries.map((entry) => (
                      <div
                        key={entry.id}
                        className={`p-2 rounded-md border text-xs ${getFocusDomainColor(entry.focus_domain)}`}
                      >
              ...
              {entries && entries.length > 0
                ? Object.entries(
                    entries.reduce((acc, e) => {
                      const domain = e.focus_domain || 'None';
              ...
              {entries
                ? entries.reduce((total, e) => {
                    if (e.start_time && e.end_time) {
                      const start = parseInt(e.start_time.split(':')[0]);
                      const end = parseInt(e.end_time.split(':')[0]);
