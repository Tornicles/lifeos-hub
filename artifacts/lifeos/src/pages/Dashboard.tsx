...
export default function Dashboard() {
  ...
  const { data: insight, isPending: insightLoading } = useDailyInsight();
  ...
  return (
    <div className="space-y-6 pb-6">
          ...
          <CardContent>
            {automationLoading ? (
              ...
              <div className="space-y-2">
                <p className="text-4xl font-bold">{automation?.ultraScore || 0}</p>
            ...
            {automationLoading ? (
              ...
              <>
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-medium">{automation?.priorityZone || "Balanced"}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-muted-foreground">7-Day Trend</span>
                  <span className="font-medium flex items-center gap-1">
                    {automation?.scoreTrend && automation.scoreTrend > 0 ? (
                      <>
                        <TrendingUp className="w-4 h-4 text-green-600" />
                        <span className="text-green-600">+{automation.scoreTrend}</span>
                      </>
                    ) : automation?.scoreTrend && automation.scoreTrend < 0 ? (
                      <>
                        <TrendingDown className="w-4 h-4 text-red-600" />
                        <span className="text-red-600">{automation.scoreTrend}</span>
                      </>
                    ) : (
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {!automationLoading && automation?.priorityHub && (
          <PriorityHubCard automation={automation} />
        )}
        
        {!automationLoading && automation?.focusRecommendations && (
          <RecommendationCard
            actions={automation.focusRecommendations.suggestedActions}
            opportunities={automation.focusRecommendations.opportunities}
            risks={automation.focusRecommendations.riskFactors}
          />
        )}
      </div>
          ...
          <CardContent className="pt-6">
            ...
            <div>
              ...
              <ul className="space-y-3">
                {automation.focusRecommendations.suggestedActions.map((action: string, idx: number) => (
