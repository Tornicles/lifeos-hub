-- 0008_learning
-- RECONCILIATION: Keep academy_* tables as canonical (already imported days 1-100).
-- Do NOT create content_modules / lessons duplicates. Master Architecture names map as:
--   content_modules -> academy_modules
--   lessons         -> academy_lessons
--   lesson_sessions -> user_lesson_progress
--   xp_events       -> user_xp_events (enhanced with idempotency_key)
--   streak_states   -> user_streaks (enhanced)
...
create table if not exists public.lesson_games (
  ...
  game_type text not null default 'strategy_scenario' check (
    game_type in ('strategy_scenario','decision_lab','ranking','budget_allocation','risk_sort','case_builder')
  ),
  title text not null,
...
-- Harden XP ledger for existing installs that lack columns
alter table public.user_xp_events add column if not exists source_type text;
alter table public.user_xp_events add column if not exists source_id uuid;
alter table public.user_xp_events add column if not exists reason text;
alter table public.user_xp_events add column if not exists idempotency_key text;
alter table public.user_xp_events add column if not exists xp_amount int;

do $$ begin
  alter table public.user_xp_events drop constraint if exists user_xp_events_event_type_check;
exception when undefined_object then null; end $$;

-- Ensure academy_modules has foundation columns on prior installs
alter table public.academy_modules add column if not exists visibility text not null default 'authenticated';
alter table public.academy_modules add column if not exists status text not null default 'published';
alter table public.lesson_cards add column if not exists media jsonb not null default '{}'::jsonb;
alter table public.quiz_questions add column if not exists difficulty text not null default 'standard';
alter table public.quiz_options add column if not exists feedback text;
alter table public.user_lesson_progress add column if not exists last_card_number int;
alter table public.user_quiz_attempts add column if not exists attempt_number int not null default 1;

create unique index if not exists user_xp_events_idempotency_uidx
...
create table if not exists public.user_streaks (
  ...
  grace_credits int not null default 0 check (grace_credits >= 0),
  timezone text not null default 'UTC',
  updated_at timestamptz not null default now()
);

alter table public.user_streaks add column if not exists grace_credits int not null default 0;
alter table public.user_streaks add column if not exists timezone text not null default 'UTC';
...
create table if not exists public.badges (
  ...
  proof_rules jsonb not null default '{}'::jsonb,
  artwork_path text,
...
create table if not exists public.content_tags (
  ...
  slug text not null unique,
  label text not null,
  taxonomy text not null check (taxonomy in ('topic','audience','risk','scripture','format')),
  created_at timestamptz not null default now()
);
...
-- Compatibility views for master-architecture naming
create or replace view public.content_modules
with (security_invoker = true) as
select * from public.academy_modules;
